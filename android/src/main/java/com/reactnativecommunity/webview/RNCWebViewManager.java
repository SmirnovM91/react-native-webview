 ...
 * topLoadingFinish - topLoadingStart - topLoadingStart - topLoadingProgress -
...
public class RNCWebViewManager extends SimpleViewManager<WebView> {
  ...
  public @Nullable Map<String, Integer> getCommandsMap() {
    Map map = MapBuilder.of("goBack", COMMAND_GO_BACK, "goForward", COMMAND_GO_FORWARD, "reload", COMMAND_RELOAD,
        "stopLoading", COMMAND_STOP_LOADING, "postMessage", COMMAND_POST_MESSAGE, "injectJavaScript",
        COMMAND_INJECT_JAVASCRIPT, "loadUrl", COMMAND_LOAD_URL);
  ...
  public void receiveCommand(WebView root, int commandId, @Nullable ReadableArray args) {
    ...
    case COMMAND_POST_MESSAGE:
      try {
        ...
        reactWebView.evaluateJavascriptWithFallback("(function () {" + "var event;" + "var data = "
            + eventInitDict.toString() + ";" + "try {" + "event = new MessageEvent('message', data);" + "} catch (e) {"
      ...
      mWebChromeClient = new RNCWebChromeClient(reactContext, webView) {
        ...
        public void onShowCustomView(View view, CustomViewCallback callback) {
          ...
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            mReactContext.getCurrentActivity().getWindow().setFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS,
  ...
  protected static class RNCWebViewClient extends WebViewClient {
    ...
    private void loadSSLCertificates(WebView view, String name) {
      try {
        CertificateFactory certificateFactory = CertificateFactory.getInstance("X.509");

        int rawId = view.getContext().getResources().getIdentifier(name, "raw", "kz.gov.kgd.android");
        InputStream inputStream = view.getContext().getResources().openRawResource(rawId);
        ...
        try {
          ...
          if (certificate instanceof X509Certificate) {
            ...
            // Log.w(kz, "Wrong Certificate format: " + rawId);
            System.out.println("Wrong Certificate format: " + rawId);
          }
        } catch (CertificateException exception) {
          // Log.w(TAG, "Cannot read certificate: " + rawId);
          System.out.println("Cannot read certificate: " + rawId);
        } finally {
          try {
    ...
    public void onReceivedSslError(WebView view, final SslErrorHandler handler, SslError error) {
      ...
      certificates.clear();
      loadSSLCertificates(view, "epaygovkz");
      loadSSLCertificates(view, "epayniteckz");
      ...
      System.out.println("SSLERROR: " + message);
      // Log.w(TAG, message);
    }
  ...
  protected static class RNCWebChromeClient extends WebChromeClient implements LifecycleEventListener {
    ...
    public void onProgressChanged(WebView webView, int newProgress) {
      ...
      if (url != null && activeUrl != null && !url.equals(activeUrl)) {
        return;
      }
      WritableMap event = Arguments.createMap();
      event.putDouble("target", webView.getId());
  ...
  protected static class RNCWebView extends WebView implements LifecycleEventListener {
    ...
    protected void onScrollChanged(int x, int y, int oldX, int oldY) {
      ...
      if (mOnScrollDispatchHelper.onScrollChanged(x, y)) {
        ScrollEvent event = ScrollEvent.obtain(this.getId(), ScrollEventType.SCROLL, x, y,
            mOnScrollDispatchHelper.getXFlingVelocity(), mOnScrollDispatchHelper.getYFlingVelocity(),
            this.computeHorizontalScrollRange(), this.computeVerticalScrollRange(), this.getWidth(), this.getHeight());
